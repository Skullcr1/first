...
atidarom:
        ...
        cmp ax, 0002h ; start of text - 02h ascii lenteleje
        je nera_failo2
        mov handler2, ax
        jmp  disasm
...
disasm:					; pagrindine vieta, cia vyksta visas procesas
	mov baitu_skaicius, 0h
	call skaitom_file
	
	cmp ax, 0
	je uzdarom			; tada viskas jau nera daugiau in nuskaityma
	call pertraukimas
...
pabaiga:
    ...
    PROC skaitom_file
        ...
        cmp ax, 0
        je skip
        mov failo_pabaiga, 0h		;jeigu nera nulis ax tada bus pabaiga, otherwise dar yra nuskaityti 
        jmp toliau1
        skip:
...
;cia nustatomas komanda
	
	mov broken, 0h				;broken ir xchg2 naudojami, nes kazkur 'w' sufuckupinta, todel kai w=0 visvien rezultate budavo 'AX'
	mov xchg2, 0h
...
;-----------------------------------------------------------------
	...
	call printAX ; isspausdina postumi nuo pradzios
	
;-----------------------------------------------------------------
	
	call printSpace
	
	call spausdinti_masinini_koda ; kodas hex 
	call printSpace
	call printSpace
	call printSpace
	;SPAUSDINAMA KOMANDA
...
;------------------------------------------------------------------
kitas_baitas:		; gaunamas kitas baitas
	push bx
	;	mov bh, 0
		inc buff_pozicija
...
;-----------------------------------------------------------------------------------------------------------------------
;-----------------------------------------------------------------------------------------------------------------------	
skaidyk_adr_baita: ;pagal AL; nustato r/m reg mod
	push ax
	push cx
		mov modas, al
...
RET
;-----------------------------------------------------------------------------------------------------------------------
;---------------------cia nustatoma tiksliai kokia komanda--------------------------------------------------------------
;-----------------------------------------------------------------------------------------------------------------------
kokia_komanda:
